...
def train(freq, data, model, guide, lr=0.001, n_steps=10000):
    pyro.clear_param_store()
    adam_params = {"lr": lr, 
                   "betas": (0.95, 0.999),
                   "eps": 1e-08}
    initial_lr = 0.001
    gamma = 10  # final learning rate will be gamma * initial_lr
    lrd = gamma ** (1 / n_steps)
    ...
    losses = []
    E_values = np.zeros(int(n_steps/50))
    rho_values = np.zeros(int(n_steps/50))
    for step in tqdm(range(n_steps)):
        loss = svi.step(freq, data)
        if step % 50 == 0:
            losses.append(loss)
            E_values[step//50] = pyro.param("E_mean").item()
            rho_values[step//50] = pyro.param("rho_mean").item()
            #print(".", end="")
            #print('[iter {}]  loss: {:.4f}'.format(step, loss))
    return lr, n_steps, losses, E_values, rho_values
...
if __name__ == "__main__":
    ...
    [lr, n_steps, losses, E_values, rho_values] = train(freq, Y_exp, model_YoungDampingDensity, guide)
