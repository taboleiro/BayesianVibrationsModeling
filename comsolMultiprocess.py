...
class ComsolMultiprocess(object):
    def __init__(self, comsolModelPath, inputFiles, freqValues):
        ...
        self.comsolModelPath = comsolModelPath
        client = mph.start()
        self.model = client.load(comsolModelPath)

    def multiprocessInit(self):
        self.jobs = Queue()
        freq = self.freqVal
        for f in freq:
            self.jobs.put(f)
    ...
    def worker(self, jobs, results):
        ...
        model = client.load(self.comsolModelPath)
        while True:
            try:
    ...
    def zzzupdateParams(self, model, freq, E, rho, eta):
        print("E: "+str(E))
        print("rho: "+str(rho))
        print("eta: "+str(eta))
        print("freq: "+str(freq))
    ...
    def zzzmodel_YoungDampingDensity(self, x, y_obs):
        ...
        E = pyro.sample("E", dist.Uniform(0.1, 10))
        ...
        with pyro.plate("data", len(y_obs)):
            ...
            for _ in range(workers):
                process = Process(target=self.worker, args=(self.jobs, self.results))
    ...
    def model_YoungDampingDensity(self, x, y_obs):
        ...
        with pyro.plate("data", len(y_obs)):
            ...
            for _ in range(len(self.freqVal)):
                process = Process(target=self.worker, args=(self.jobs, self.results))
    ...
    def run(self):
        ...
        input_x = torch.tensor(freq[(self.freqVal*2).astype(int)])
        y_obs = torch.tensor(Y_exp[(self.freqVal*2).astype(int)]) # Suppose this was the vector of observed y's
...
if __name__ == "__main__":
    ...
    freqValues = [[117.5, 122.5, 0.5],\
                  ...
                  [2977.5, 2981.5, 0.5]] 
    freqValues = [[60, 600, 1]]
    obj = ComsolMultiprocess("comsol/beam.mph", files, freqValues)
    obj.run()
